...
- **Supabase**: project `rqotdhrptadhsuvjhcoa`, tabel `anggota`, `tugas`, `tugas_pj`, `google_tokens`. RLS terbuka untuk `anggota`/`tugas`/`tugas_pj` (app belum ada login), `google_tokens` sengaja default-deny (cuma lewat `service_role` key di server)
